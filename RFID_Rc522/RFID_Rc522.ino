/*
 * http://www.arduino.cn/thread-4560-1-1.html
 * RC522
 * SDA    SCK   MOSI    MISO    IRQ   GND   RST   3.3v 
 * 10     13    11      12      空    GND   5     3.3V
 *外界一个共阳极LED
 * 8 9 接另外两个脚
 * 达到刷钥匙扣和白卡分别控制红绿亮起效果
 *钥匙扣35D10902
...
void loop()
{
    ...
    if (status == MI_OK)
    {
        ...
        uchar* id = serNum;
        if( id[0]==0x35 && id[1]==0xd1 && id[2]==0x09 && id[3]==0x02 ) {
            Serial.println("Hello Yaoshikou!");
            digitalWrite(RedLedPin,LOW);
            digitalWrite(GreenLedPin,HIGH);
        } else if(id[0]==0xa4 && id[1]==0x46 && id[2]==0x6f && id[3]==0xd5) {
            Serial.println("Hello Baika!");
